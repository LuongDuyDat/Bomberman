...
public class Oneal extends Enemy{
    ...
    public int generateDirect() {
        ...
        while (q.peek() != null) {
            ...
            if (i > 2 && d[i - 1][j] > d[i][j] + 1 && !(map[i - 1][j] instanceof Wall) && !(map[i - 1][j] instanceof Brick)) {
                q.add((i - 1) * _width + j);
                d[i - 1][j] = d[i][j] + 1;
                trace[i - 1][j] = i * _width + j;
            }
            if (i + 1 < _height + 2 && d[i + 1][j] > d[i][j] + 1 && !(map[i + 1][j] instanceof Wall) && !(map[i + 1][j] instanceof Brick)) {
                q.add((i + 1) * _width + j);
                d[i + 1][j] = d[i][j] + 1;
                trace[i + 1][j] = i * _width + j;
            }
